...
import { requireAuthOrGuest } from '../middleware/requireAuthOrGuest.js';
...
function repoFilter(req) {
  return req.isGuest
    ? { _id: req.params.indexId, guestSessionId: req.guestSessionId }
    : { _id: req.params.indexId, userId: req.userId };
}

// ── POST /api/repos/index ─────────────────────────────────────────────────────
router.post('/index', requireAuthOrGuest, async (req, res) => {
  ...
  const repoData = { githubUrl, status: 'pending' };
  if (req.isGuest) {
    repoData.guestSessionId = req.guestSessionId;
  } else {
    repoData.userId = req.userId;
  }

  const repo = await IndexedRepo.create(repoData);
...
router.get('/:indexId/status', requireAuthOrGuest, async (req, res) => {
  const repo = await IndexedRepo.findOne(repoFilter(req));
...
router.post('/:indexId/query', requireAuthOrGuest, guestRateLimit, async (req, res) => {

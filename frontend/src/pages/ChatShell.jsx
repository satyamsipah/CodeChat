...
import { buildHeaders, clearGuestSession, getGuestSessionId } from '../utils/auth.js';
...
export default function ChatShell() {
  ...
  const isGuestSession = !!getGuestSessionId();
  ...
  const pollStatus = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/api/repos/${indexId}/status`, {
  ...
  return (
    <div className="flex h-screen bg-slate-900 text-slate-200">
      ...
      <aside className="w-64 bg-slate-800 border-r border-slate-700 flex flex-col p-4 shrink-0">
        <Link to="/" className="text-lg font-semibold text-white mb-4 hover:text-violet-300 transition-colors">
          CodeChat
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isGuestSession ? 'Exit Guest' : 'Logout'}
          </button>
        </div>
      </aside>
